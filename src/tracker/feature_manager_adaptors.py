...
class PyramidAdaptor(object):
    def __init__(self,
        ...
        if self.use_block_adaptor:
            self.block_adaptor = BlockAdaptor(
                self.detector, self.descriptor, row_divs=kAdaptorNumRowDivs, col_divs=kAdaptorNumColDivs, do_parallel=False)
    ...
    def initSigmaLevels(self):
        ...
        self.num_features_per_level = np.zeros(num_levels, dtype=int)
