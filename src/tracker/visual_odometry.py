...
class VoStage(Enum):
    ...
    # got first image, we can proceed in a normal way (match current image with previous image)
    GOT_FIRST_IMAGE = 1


kVerbose = False
kMinNumFeature = 2000
# metric threshold used for normalized image coordinates
kRansacThresholdNormalized = 0.0003
kRansacThresholdPixels = 0.1         # pixel threshold used for image coordinates
# absolute translation scale; it is also the minimum translation norm for an accepted motion
kAbsoluteScaleThreshold = 0.1
# using the essential matrix fitting algorithm is more robust RANSAC given five-point algorithm solver
...
class VisualOdometry(object):
    def __init__(self, cam, groundtruth, feature_tracker: FeatureTracker):
        ...
        self.timer_pose_est = TimerFps(
            'PoseEst', is_verbose=self.timer_verbose)
    ...
    def getAbsoluteScale(self, frame_id):
        if self.groundtruth is not None and kUseGroundTruthScale:
            self.trueX, self.trueY, self.trueZ, scale = self.groundtruth.getPoseAndAbsoluteScale(
                frame_id)
            return scale
        else:
            self.trueX = 0
    ...
    def computeFundamentalMatrix(self, kps_ref, kps_cur):
        F, mask = cv2.findFundamentalMat(
            kps_ref, kps_cur, cv2.FM_RANSAC, param1=kRansacThresholdPixels, param2=kRansacProb)
    ...
    def estimatePose(self, kps_ref, kps_cur):
        ...
        if kUseEssentialMatrixEstimation:
            ...
            E, self.mask_match = cv2.findEssentialMat(self.kpn_cur, self.kpn_ref, focal=1, pp=(
                0., 0.), method=ransac_method, prob=kRansacProb, threshold=kRansacThresholdNormalized)
        else:
            # just for the hell of testing fundamental matrix fitting ;-)
            F, self.mask_match = self.computeFundamentalMatrix(
                kp_cur_u, kp_ref_u)
        ...
        _, R, t, mask = cv2.recoverPose(
            E, self.kpn_cur, self.kpn_ref, focal=1, pp=(0., 0.))
        return R, t  # Rrc, trc (with respect to 'ref' frame)

    # @profile
    def processFirstFrame(self):
        # only detect on the current image
        self.kps_ref, self.des_ref = self.feature_tracker.detectAndCompute(
            self.cur_image)
    ...
    def processFrame(self, frame_id):
        ...
        if (absolute_scale > kAbsoluteScaleThreshold):
            ...
            # print('estimated t with norm |t|: ', np.linalg.norm(
            #     t), ' (just for sake of clarity)')
        ...
        if (self.feature_tracker.tracker_type == FeatureTrackerTypes.LK) and (self.kps_ref.shape[0] < self.feature_tracker.num_features):
            self.kps_cur, self.des_cur = self.feature_tracker.detectAndCompute(
                self.cur_image)
            # convert from list of keypoints to an array of points
    ...
    def track(self, img, frame_id):
        ...
        print("time:", self.timer_main.elapsed())
