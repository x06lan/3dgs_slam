...
from tracker.feature_manager import feature_manager_factory
...
class FeatureTracker(object):
    def __init__(self, num_features=kMinNumFeatureDefault,
                 # number of pyramid levels for detector and descriptor
                 num_levels=1,
                 # detection scale factor (if it can be set, otherwise it is automatically computed)
                 scale_factor=1.2,
                 detector_type=FeatureDetectorTypes.FAST,
        ...
        # it contain descriptors matching methods based on BF, FLANN, etc.
        self.matcher = None
...
class LkFeatureTracker(FeatureTracker):
    def __init__(self, num_features=kMinNumFeatureDefault,
        ...
        Printer.green(
            'LkFeatureTracker: num levels on LK pyr optic flow: ', optic_flow_num_levels)
    ...
    def track(self, image_ref, image_cur, kps_ref, des_ref=None):
        kps_cur, st, err = cv2.calcOpticalFlowPyrLK(
            image_ref, image_cur, kps_ref, None, **self.lk_params)  # shape: [k,2] [k,1] [k,1]
...
class DescriptorFeatureTracker(FeatureTracker):
    def __init__(self, num_features=kMinNumFeatureDefault,
                 # number of pyramid levels for detector
                 num_levels=1,
                 # detection scale factor (if it can be set, otherwise it is automatically computed)
                 scale_factor=1.2,
                 detector_type=FeatureDetectorTypes.FAST,
        ...
        if tracker_type == FeatureTrackerTypes.DES_FLANN:
            ...
            raise ValueError(
                "Unmanaged matching algo for feature tracker %s" % self.tracker_type)

        # init matcher
        self.matcher = feature_matcher_factory(
            norm_type=self.norm_type, ratio_test=match_ratio_test, type=self.matching_algo)
    ...
    def track(self, image_ref, image_cur, kps_ref, des_ref):
        ...
        res.kps_ref_matched = np.asarray(
            kps_ref[idxs_ref])  # the matched ref kps
        res.idxs_ref = np.asarray(idxs_ref)

        res.kps_cur_matched = np.asarray(
            kps_cur[idxs_cur])  # the matched cur kps
