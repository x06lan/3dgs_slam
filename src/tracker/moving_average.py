...
class MovingAverage:
    ...
    def init(self, initVal=None):
        if initVal is None:
            initVal = 0.
        self._ring_buffer = np.full(
            self._average_width, initVal, dtype=np.float32)
    ...
    def getAverage(self, new_val=None):
        ...
        if self._is_compute_sigma:
            self._sigma2 = self._sigma2 + self._one_over_average_width_min_one*(self._average_width*(
                averageOld*averageOld - self._average*self._average) - oldVal*oldVal + new_val*new_val)
