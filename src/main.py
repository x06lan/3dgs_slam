...
class Tracker:
    ...
    def init(self):
        ...
        if self.test:
            ...
            self.trainer = Trainer(ckpt="3dgs_slam_ckpt.pth", camera=self.camera, lr=self.lr, downsample=self.downsample, distance=self.distance)
        else:
            ...
            self.dataset = ColmapDataset("dataset/nerfstudio/poster", downsample_factor=self.downsample)
            self.camera = self.dataset.camera
            self.trainer = Trainer(camera=self.camera, lr=self.lr, downsample=self.downsample)
