...
class _Drawer(torch.autograd.Function):
    ...
    def forward(
        ...
        rendered_image = torch.zeros(
            padded_height, padded_width, 5, device=gaussians_pos.device, dtype=torch.float32)
        gaussian_cuda.draw(
            gaussians_pos,
            gaussians_rgb,
            gaussians_opa,
        ...
        ctx.save_for_backward(gaussians_pos, gaussians_rgb, gaussians_opa, gaussians_cov,
                              tile_n_point_accum, rendered_image, rays_o, lefttop_pos, vec_dx, vec_dy)
        ctx.focal_x = focal_x
        ctx.focal_y = focal_y
...
class _GlobalCulling(torch.autograd.Function):
    ...
    def forward(ctx, pos, quat, scale, current_rot, current_tran, near, half_width, half_height):
        ...
        ctx.save_for_backward(culling_mask, pos, quat,
                              scale, current_rot, current_tran)
