...
from utils.utils_geom import add_ones, homography_matrix
from utils.utils_draw import draw_random_img
...
def img_blocks(img, row_divs, col_divs):
    ...
    xs = np.uint32(np.rint(np.linspace(0, cols, num=col_divs+1)))
...
def img_mask_blocks(img, mask, row_divs, col_divs):
    ...
    for y1, y2 in zip(ystarts, yends):
        for x1, x2 in zip(xstarts, xends):
            ...
            yield img[y1:y2, x1:x2], mask_block(mask, x1, x2, y1, y2), y1, x1
...
def pyramid(image, scale=1.2, minSize=(30, 30), gauss_filter=True, sigma0=1.0):
    ...
    # no filtering on the original image from https://www.vlfeat.org/api/sift.html#sift-tech-ss
    sigma_nominal = 0.5
    ...
    while True:
        ...
        if gauss_filter:
            ...
            image = cv2.resize(blur, (0, 0), fx=inv_scale, fy=inv_scale)
        else:
            # ,interpolation = cv2.INTER_NEAREST)
            image = cv2.resize(image, (0, 0), fx=inv_scale, fy=inv_scale)
...
def transform_img(img, rotx, roty, rotz, tx=0, ty=0, scale=1, adjust_frame=True):
    ...
    if adjust_frame:
        ...
        transformed_img_box = (H @ add_ones(img_box).T)
        transformed_img_box = (
            transformed_img_box[:2]/transformed_img_box[2]).T
        transformed_center = (H @ center.T).T
    else:
...
def add_background(img, img_box, img_background=None):
    if img_background is None:
        ...
        if img_background.shape != img.shape:
            ...
            if img.ndim == 2:
                img_background = cv2.cvtColor(
                    img_background, cv2.COLOR_RGB2GRAY)
        # print('img.shape:',img.shape,', img_background.shape:',img_background.shape)
    mask = mask_from_polygon(img.shape, img_box)
    inverse_mask = cv2.bitwise_not(mask)
    img_background = cv2.bitwise_or(
        img_background, img_background, mask=inverse_mask)
...
def remove_borders(image, borders):
    ...
    if len(shape) == 4:
        ...
        new_im[:, borders:shape[0]-borders, borders:shape[1]-borders,
               :] = image[:, borders:shape[0]-borders, borders:shape[1]-borders, :]
    elif len(shape) == 3:
        new_im[borders:shape[0] - borders, borders:shape[1] - borders,
               :] = image[borders:shape[0] - borders, borders:shape[1] - borders, :]
    else:
        new_im[borders:shape[0] - borders, borders:shape[1] -
               borders] = image[borders:shape[0] - borders, borders:shape[1] - borders]
    return new_im
