...
def poseRt(R, t):
    ...
    ret[:3, :3] = R
    # ret[:3, 3] = t
    ret[:3, 3] = np.array([elem[0] for elem in t])
    return ret
...
def computeF12(f1, f2):
    ...
    R21 = R12.T
    # infinite homography from f1 to f2 [Hartley Zisserman pag 339]
...
def check_dist_epipolar_line(kp1, kp2, F12, sigma2_kp2):
    ...
    # squared (minimum) distance of kp2 from the epipolar line l
...
def estimate_pose_ess_mat(kpn_ref, kpn_cur, method=cv2.RANSAC, prob=0.999, threshold=0.0003):
    ...
    E, mask_match = cv2.findEssentialMat(kpn_cur, kpn_ref, focal=1, pp=(
        0., 0.), method=method, prob=prob, threshold=threshold)
