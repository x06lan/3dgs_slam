...
from utils.utils_geom import add_ones, s1_diff_deg, s1_dist_deg, l2_distances
...
def unpackSiftOctaveIntra(kpt):
    ...
    if octave >= 0:
        ...
        scale = float(1 << (-octave))
    # return a virtual intra-level with a virtual scale factor =  2^(1/3) where 3 is the number of layers per octave
    return (octave+1)*3 + layer
...
def sat_num_features(kps, des=None, num_features=Parameters.kNumFeatures):
    if len(kps) > num_features:
        ...
        if des is None:
            ...
            # print('sat with des')
            order = np.argsort([kp.response for kp in kps])[
                ::-1][:num_features]      # [::-1] is for reverse order
...
def ssc_nms(kps, des, cols, rows, num_ret_points=Parameters.kNumFeatures, tolerance=0.1):
    ...
    while not complete:
        ...
        covered_vec = [[False for _ in range(
            num_cell_cols + 1)] for _ in range(num_cell_cols + 1)]
        result = []

        for i in range(len(kps)):
            # get position of the cell current point is located at
            row = int(math.floor(kps[i].pt[1] / c))
            col = int(math.floor(kps[i].pt[0] / c))
            if not covered_vec[row][col]:  # if the cell is not covered
                result.append(i)
                # get range that current radius is covering
                row_min = int((row - math.floor(width / c))
                              if ((row - math.floor(width / c)) >= 0) else 0)
                row_max = int(
                    (row + math.floor(width / c)) if (
                        (row + math.floor(width / c)) <= num_cell_rows) else num_cell_rows)
                col_min = int((col - math.floor(width / c))
                              if ((col - math.floor(width / c)) >= 0) else 0)
                col_max = int(
...
def octree_nms(frame, kps, num_features):
    ...
    minY = 0
    maxY = frame.shape[0]
    kps_tuples = [(kp.pt[0], kp.pt[1], kp.size, kp.angle,
                   kp.response, kp.octave) for kp in kps]
    kps_tuples = ORBextractor.DistributeOctTree(
...
def grid_nms(kps, des, H, W, num_features, dist_thresh=4):
    ...
    """
    in_corners = np.array([(kp.pt[0], kp.pt[1], kp.response)
                          for kp in kps]).T  # 3xN [x_i,y_i,conf_i]^T
...
def nms_from_map(score_map, size):
    ...
    # cv2.dilate is a maximum filter
    score_map = score_map * (score_map == cv2.dilate(score_map, kernel))
    return score_map
...
def get_best_points_coordinates(score_map, num_points=1000, threshold=-1):
    indexes = get_best_score_idxs(
        score_map, num_points=num_points, threshold=threshold)
    coords = []
    for idx in indexes:
...
def extract_patches_tensor(img, kps, patch_size=32, mag_factor=1.0, warp_flags=cv2.WARP_INVERSE_MAP + cv2.INTER_CUBIC + cv2.WARP_FILL_OUTLIERS):
    patches = np.ndarray(
        (len(kps), 1, patch_size, patch_size), dtype=np.float32)
    ...
    for i, kp in enumerate(kps):
        ...
        M = np.matrix([
            [+scale_cos, -scale_sin,
                (-scale_cos + scale_sin) * half_patch_size + x],
            [+scale_sin, +scale_cos, (-scale_sin - scale_cos) * half_patch_size + y]])

        patch = cv2.warpAffine(
            img, M, (patch_size, patch_size), flags=warp_flags)
