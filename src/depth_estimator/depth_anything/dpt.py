...
class DPT_DINOv2(nn.Module):
    def __init__(self, encoder="vitl", features=256, out_channels=[256, 512, 1024, 1024], use_bn=False, use_clstoken=False, localhub=True):
        ...
        if localhub:
            self.pretrained = torch.hub.load("src/depth_estimator/depth_anything/torchhub/facebookresearch_dinov2_main", "dinov2_{:}14".format(encoder), source="local", pretrained=False)
        else:
            self.pretrained = torch.hub.load("src/depth_estimator/depth_anything/facebookresearch/dinov2", "dinov2_{:}14".format(encoder))
